...
!(function($){
	...
	var Sandbox,
		mLoader = jEngine.Core.ModuleLoader,
		configs = {
		threshold:250,
		end:0
	};
			...
			*/
					...
					if(event === "exposure"){				
						...
						var handle = function(e) {
							...
							timeId = setTimeout(function(){
								self._getModule(element, config, callback, event);
								doms.unbind(event, handle);
								if(event == "mouseover"){
									doms.unbind("mouseout", clearTime);
								}
							}, 50);
						};
						...
						//解决鼠标快速滑过节点引起的事件。
						var clearTime = function(){clearTimeout(timeId);};
						if(event == "mouseover"){
							doms.bind("mouseout", clearTime);
						}
					}
				}
				else{
			...
			_getModule:function(el, cfg, fn, event){
				...
				if(module){
					...
					mLoader.add(moduleId, {
							js: module.js,
							css: module.css
					});
					if(fn){
						mLoader.use(moduleId, function(){
							fn(el);
							if(event){
