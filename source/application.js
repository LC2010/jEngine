...
!(function($){
	...
	var configs = {
		...
		docField:     'data-doc-config',	//server端的全局变量通过这个属性埋入到模块对应doc节点中，框架会负责取出节点中数据并传给模块的this.config.global变量。
    ...
    $.extend(AppEntity.prototype, {
		...
		*/
		register: function(moduleId, creator, opt){
			if (opt == null) opt = {};
			try {
			  if(!this._addModule(moduleId, creator, opt))
				return false;
			  ...
			  return true;
				...
				if(!DEBUG_MOD){
					$.logger.error("could not register " + moduleId + " because: " + ex.message, moduleId);
				}else{
					throw ex;
				}
			  return false;
		...
		*/
		lazyRegister: function(moduleId, creator, els, event, opt){
			...
			try {
				...
				if(!DEBUG_MOD){
					$.logger.error("could not lazyregister " + moduleId + " because: " + ex.message, moduleId);
				}else{
					throw ex;
				}
				return false;
		...
		*/
		start: function(moduleId){
			...
			try {
				...
				$.logger.info(moduleId + " init finished, cost:" + + ($.now() - start) + " ms", moduleId);
				return true;
			} catch (ex) {
				if(!DEBUG_MOD){
					$.logger.error(moduleId + " init Error: " + ex.message, moduleId);
				}else{
					throw ex;
				}
			  return false;
		...
		*/		
		...
		_addModule: function(moduleId, creator, opt) {
			...
			if(typeof creator === "string"){
 			...
 			if (typeof creator !== "function") {
				//use方法有script加载失败的情况，暂时忽略这些异常。
				if(opt.ignore){
					return false;
				}else{
					throw new Error("creator "+ original +" has to be a constructor function");
				}
			}	
						...
						instance[name] = function(name, method){
							return function (){
								try { 
									...
									$.logger.error(moduleId + " throw error: " +  name + "()-> " + ex.message, moduleId);
								}
							};
						}(name, method);
					}
		...
		_lazyStart:function(moduleId, creator){
			...
			if (this.moduleData[moduleId] == null) {
				if(this.register(moduleId, creator, {ignore:true})){
					return this.start(moduleId);
				}
			}
			return false;
		...
		_getModuleConfig:function(moduleId, opt){
			...
			if(!this.docDetect){
				try{
					var doc = $('#doc');
					if(doc.length>0){
						var data = doc.attr(this.config.docField);
						if(data && data.trim()){
							data = data.replace(/\\'/g, "'");
							this.config.global = opt.global = $.parseJSON(data);
						}
					}
				}catch(e){
				}
    ...
    AppCore = jEngine.Core.AppEntity = new AppEntity();
